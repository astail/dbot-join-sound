...
- **チャットの読み上げ**: Bot が参加中の VC に付いているテキストチャットの発言を、本文だけ VOICEVOX で読み上げる（`VC_CHAT_READ_ENABLED=false` で無効）。`/shaberu-ch add` を実行したチャンネルの発言も読み上げる（Bot が通話にいる間だけ）。画像や動画は「画像がアップロードされました」のように知らせる
- **VC 付属チャットの自動削除**: Bot が参加中の VC に付いているテキストチャットの発言を、Bot の返信も含めて 30 秒後に削除。Bot のいない VC と普通のテキストチャンネルは消さない（`VC_CHAT_DELETE_SECONDS` で変更、`0` で無効）
...
- **通話外では黙る**: Bot がどの通話にもいないときは、チャットに何も書き込まない。`@Bot join` 以外のコマンドは実行もしない
...
`applications.commands` はスラッシュコマンド（`/yomi` と `/shaberu-ch`）の登録に必要です。すでに `scope=bot` だけで招待済みのサーバーでも、この URL を踏み直せばスコープが追加されます。Bot は退出せず、登録済みの入室音も残ります。
...
| `VC_CHAT_READ_ENABLED` | `true` | `false` で VC 付属チャットを読み上げない（`/shaberu-ch` で指定したチャンネルは読む） |
...
Bot がそのサーバーのどの通話にもいないときは、チャットに何も書き込みません。結果を返せないまま登録や削除だけが起きないよう、コマンドも実行しません（`@Bot join` だけは通話に参加します。参加できなくても返信はしません）。入室音の登録や `/yomi` などを使うときは、先に誰かが VC に入って Bot を参加させてください。スラッシュコマンドは Discord の仕様上なにか応答する必要があるため、本人にだけ見える一時メッセージで「使えません」と返します。
...
- Bot が参加中の VC に付いているテキストチャット（設定不要。`VC_CHAT_READ_ENABLED=false` で読まなくなる）
- `/shaberu-ch add` を実行したチャンネル

```text
/shaberu-ch add      このチャンネルのチャットを読み上げる
/shaberu-ch remove   このチャンネルのチャットを読み上げない
/shaberu-ch list     読み上げるチャンネルの一覧（VC 付属チャットは含まない）
```

- 入退室の音と同じ順番待ちに並ぶので、重なって鳴ることはない
- URL は「URL」、伏せ字（`||…||`）は「伏せ字」と読み、カスタム絵文字は読まない。100 文字を超える部分は「以下略」で打ち切る
- 画像や動画が添付されていると「画像がアップロードされました」「動画がアップロードされました」（両方なら「画像と動画がアップロードされました」）と、本文に続けて読む。それ以外のファイルは読まない
- `/yomi` で登録した読み方はチャットにも効く
- Bot の発言と、Bot へのメンション（`@Bot check` などのコマンド）は読まない
- 読み上げるチャンネルは `sounds/shaberu-ch.json` に保存され、全サーバー共通の 1 ファイルで持つ

Bot が参加中の VC の付属チャットは、発言から 30 秒で自動的に削除されます。消すかどうかは投稿した時点で決まるので、投稿のあと 30 秒以内に Bot が抜けても予定どおり消えます。Bot が参加していない VC の付属チャットと、普通のテキストチャンネルの発言は消しません。Bot の返信も消えるため、VC 付属チャットで `@Bot delete` すると、控えとして添付される削除した音声も 30 秒で消えます。控えを残したいときは普通のテキストチャンネルで実行してください。削除の予約はメモリ上に持つので、30 秒以内に Bot を再起動した発言は消えずに残ります。
